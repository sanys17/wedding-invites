...
import { getTemplateComponent, TEMPLATE_REGISTRY } from "./templates";
...
export default function InvitePreview({ data, animate = false }: Props) {
  ...
  const templateMeta = TEMPLATE_REGISTRY.find((r) => r.id === (data.template || "elegant-minimal"));
  const isHorizontal = templateMeta?.orientation === "horizontal";
  ...
  return (
    <div
      className={`relative w-full mx-auto shadow-xl overflow-hidden ${animate ? "fade-in" : ""} ${isHorizontal ? "max-w-xl" : "max-w-md"}`}
